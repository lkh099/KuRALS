...
class FCNHead(BaseDecodeHead):
    ...
    def __init__(self, in_channels=512, in_index=2, channels=256, num_convs=1, concat_input=False, dropout_ratio=0.1, num_classes=3,
        ...
        convs = []
        convs.append(
            ConvBlock(in_ch=self.in_channels,
        ...
        if self.concat_input:
            self.conv_cat = ConvBlock(
                      in_ch=self.in_channels + self.channels,
                      out_ch=self.channels,
                      k_size=kernel_size,
                      pad=kernel_size//2,
                      dil=1)
    ...
    def losses(self, seg_logit, seg_label):
        ...
        loss = dict()
        # seg_logit = F.interpolate(input=seg_logit, size=seg_label.shape[2:], align_corners=self.align_corners, mode='bilinear')
        seg_logit = resize(
            input=seg_logit,
